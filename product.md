...
4. **AI verdicts with investigation** — the model fetches release notes, changelogs, and CVE details itself (read-only tools) before judging breaking risk in the version gap.
5. **Cost-aware model routing** — a cheap deployment for routine verdicts; automatic escalation to a stronger one for major-version gaps, grouped PRs, or low-confidence first passes. Both are Azure deployment names and may point at the same deployment.
...
9. **Token monitoring** — each run reports total tokens consumed (input/output breakdown), LLM API calls, and cache hits directly in the digest and structured logs, making per-run cost visible at a glance.
10. **Keyless Azure auth** — Entra ID via `DefaultAzureCredential` (managed identity in Azure, `az login` locally) with a fresh bearer token per request; a resource key remains available for local testing.
11. **Azure-ready** — Timer-triggered Function wrapper and a documented deployment path (`MIGRATION.md`), plus `scripts/smoke_test.py` to verify endpoint, auth, and deployment names before a run.
12. **Deployment-agnostic requests** — deployments disagree on `temperature` and `max_tokens` vs `max_completion_tokens`; the client detects the rejection, adapts, and remembers it per deployment, so one code path spans model generations.

## Guardrails

1. **Report-only** — v1 never writes to any PR: no comments, no closes, no merges.
2. **Read-only AI tools** — the model can only fetch release notes, CVE details, and files; every tool result is size-capped.
3. **Tool loop cap** — maximum 5 tool iterations per PR, then a final answer is forced.
4. **Hard cost circuit breaker** — configurable cap (default 100) on LLM API calls per run.
...
9. **Secrets hygiene** — `.env` and all run outputs are gitignored; keyless Entra ID auth means no inference secret exists to leak. Data-plane RBAC is separate from control plane: an identity needs Cognitive Services OpenAI User on the resource, which subscription Owner does not grant.
...
5. **Breaking-change check** — release notes across the version gap are read (by the model) to judge upgrade risk from evidence, not version numbers alone.
